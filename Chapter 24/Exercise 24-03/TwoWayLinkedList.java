...
class TwoWayLinkedList<E> extends java.util.AbstractSequentialList<E> {
	...
	public void addFirst(E e) {
		...
		if (head != tail) {  // For a two-way linked list, make the next item's previous element the head
			Node<E> temp = head;
			temp = head.next;
			temp.previous = head;
		}
	...
	public void addLast(E e) {
		...
		tail.previous = temp;// For a two-way linked list, link the tail's previous element to temp
	}
	...
	public void add(int index, E e) {
		if (index == 0) {
			...
			temp.previous = current.next;  // For a two-way linked list, link temp's previous element to current's next element
			(current.next).previous = current;  // For a two-way linked list, link current's next's previous element to current
		}
	}
